...
uint8_t ax25_frame_out[AX25_MAX_LEN + 1];

int ax25_frame(char *scallsign, char *dcallsign, char *path1, char *path2, char *data, ...)
{
	...
	s = _ax25_callsign(ax25_frame_out, dcallsign);
	...
	*(s++) = 0xF0; /* Protocol ID: 0xF0 = no layer 3 data */
	...
	vsnprintf((char *) s, AX25_MAX_LEN - (s - ax25_frame_out) - 2 + 1, data, va);
	va_end(va);
	
	/* Calculate and append the checksum */
	for(x = 0xFFFF, s = ax25_frame_out; *s; s++)
